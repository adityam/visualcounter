...
\startuseMPgraphic{visualcounter:pulseline}
  ...
  past_pulse   := origin -- ( (n-1)*width, 0 ) randomized 0.3width ;

  active_pulse := (((n-1)*width, 0) -- ((n-0.5)*width, -0.2height) 
                  -- (n*width, height) -- ((n+0.5)*width, -0.2height) 
                  -- ((n+1)*width,0))
                  randomized 0.3width ;

  future_pulse := ((n+1)*width, 0) -- ((m+1)*width, 0) randomized 0.3width ;


  pickup pencircle scaled \visualcounterparameter\c!rulethickness\space ;
  draw past_pulse   withcolor \MPcolor{past}   ;
  draw active_pulse withcolor \MPcolor{active} ;
  draw future_pulse withcolor \MPcolor{future} ;
