...
These settings are changed using \command{\letterbackslash setupvisualcounter}. In particular, to
...
\starttabulate[|l|l|]
  ...
  \NC Item group numbers 
      \NC \command{\letterbackslash v_strc_itemgroups_counter}\footnote[internal]
          ...
          \options{counter} to \options{\letterbackslash v_strc_itemgroups_counter} is to use:
...
All counters use three colors: \options{past} to display past counters,
\options{active} to display current counter, and \options{future} to display
future counters. To change the color scheme, first define a new color palette
(which is misspelled in \CONTEXT{} is \options{colorpalet}):
