...
This counter has the following tunable parameters:
\startitemize[packed]
  ...
  \item \options{angle} (default \options{75}): the angle of the forward
    markers. The angle of the backward marker is $180 - \command{angle}$.
\stopitemize
...
Note that if the angle is greater than \options{90}, then the direction of the
strokes change. For example the output with \options{width=1.5bp, angle=-45} is:
